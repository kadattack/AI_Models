...
# Define the Lightning Module for CNN
class CNNRegressor(pl.LightningModule):
    def __init__(self, input_dim, num_filters=64, kernel_size=3, dropout=0.1, lr=1e-3):
        super(CNNRegressor, self).__init__()
        self.save_hyperparameters()

        # Define Convolutional Layers with padding to preserve size
        ...
        conv_output_size = self._get_conv_output_size(input_dim, kernel_size)
    ...
    def _get_conv_output_size(self, input_size, kernel_size):
        # Padding is applied, so the output size remains the same after each convolution
        return input_size * (64 * 2)  # 64 is the number of filters in conv1, and 64*2 in conv2

    def forward(self, x):
        """
        x: Tensor of shape (batch_size, input_dim)
        """
        x = x.unsqueeze(1)  # (batch_size, 1, input_dim) - needed for Conv1d
