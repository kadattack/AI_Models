...
class RNNRegressor(pl.LightningModule):
    def __init__(self, input_dim, hidden_dim=128, num_layers=3, dropout=0.1, lr=1e-3):
        ...
        # Define the output layer (final fully connected layer)
        self.fc_out = nn.Linear(hidden_dim, 1)

        self.criterion = nn.MSELoss()  # Loss function for regression tasks

    def forward(self, x):
        """
        x: Tensor of shape (batch_size, input_dim)
        """
        # Reshape input to have an additional sequence length dimension (sequence length = 1 for non-sequential data)
        x = x.unsqueeze(1)  # (batch_size, 1, input_dim)

        # Forward pass through LSTM
        _, (hidden, _) = self.lstm(x)  # hidden state from the last layer is used

        # Use hidden state for prediction
        output = self.fc_out(hidden[-1])  # (batch_size, 1)
